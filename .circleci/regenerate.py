            ...
            for python_version in PYTHON_VERSIONS:
                for cu_version in CU_VERSIONS_DICT[os_type]:
                    ...
                    if not fb and (
                        ...
                        fb = "/.*/"
                    w += build_workflow_pair(
                        btype, os_type, python_version, cu_version, fb, prefix, upload
                    )
...
def build_workflow_pair(
    btype, os_type, python_version, cu_version, filter_branch, prefix="", upload=False
):

    w = []
    base_workflow_name = (
        f"{prefix}binary_{os_type}_{btype}_py{python_version}_{cu_version}"
    )
    w.append(
    ...
    if upload:
        ...
        w.append(
            generate_upload_workflow(
                base_workflow_name, filter_branch, os_type, btype, cu_version
            )
        )
...
def generate_base_workflow(
    base_workflow_name, python_version, cu_version, filter_branch, os_type, btype
):
    ...
    if btype == "conda":
        d[
            "conda_docker_image"
        ] = f'pytorch/conda-builder:{cu_version.replace("cu1","cuda1")}'
    elif cu_version.startswith("cu"):
        d[
            "wheel_docker_image"
        ] = f'pytorch/manylinux-{cu_version.replace("cu1","cuda1")}'
...
def generate_upload_workflow(
    ...
    d = {
        "name": "{base_workflow_name}_upload".format(
...
def generate_smoketest_workflow(
