...
label_template = os.path.join(args.root_dir, "labels", f"{dataset}_{subset}_transcript_lengths_seg{seg_duration}s.{args.groups}")
...
dst_label_filename = os.path.join(args.root_dir, dataset, f"{dataset}_{subset}_transcript_lengths_seg{seg_duration}s.csv")

with open(dst_label_filename, "w") as file:
    file.write('\n'.join(lines))

# Print the number of files and total duration in hours
total_duration = sum(int(line.split(',')[2]) for line in lines) / 3600. / 25.
