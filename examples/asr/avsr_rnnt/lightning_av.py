...
class AVConformerRNNTModule(LightningModule):
    ...
    def _step(self, batch, _, step_type):
        ...
        output, src_lengths, _, _ = self.model(self.fusion(torch.cat([video_features, audio_features], dim=-1)), batch.video_lengths, prepended_targets, prepended_target_lengths)
    ...
    def configure_optimizers(self):
        self.warmup_lr_scheduler = WarmupCosineScheduler(self.optimizer, 10, self.args.epochs, len(self.trainer.datamodule.train_dataloader())/self.trainer.num_devices/self.trainer.num_nodes )
    ...
    def forward(self, batch: AVBatch):
        ...
        hypotheses = decoder(self.fusion(torch.cat([video_features, audio_features], dim=-1)), batch.video_lengths.to(self.device), beam_width=20)
