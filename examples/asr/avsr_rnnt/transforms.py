...
class TrainTransform:
    ...
    def __call__(self, samples: List):
        ...
        if self.args.md == "a":
            audios, audio_lengths = _extract_features(self.train_video_pipeline, self.train_audio_pipeline, samples, self.args)
            return Batch(audios, audio_lengths, targets, target_lengths)
        if self.args.md == "v":
            videos, video_lengths = _extract_features(self.train_video_pipeline, self.train_audio_pipeline, samples, self.args)
            return Batch(videos, video_lengths, targets, target_lengths)
        if self.args.md == "av":
            audios, videos, audio_lengths, video_lengths = _extract_features(self.train_video_pipeline, self.train_audio_pipeline, samples, self.args)
...
class ValTransform:
    ...
    def __call__(self, samples: List):
        targets, target_lengths = _extract_labels(self.sp_model, samples)
        if self.args.md == "a":
            audios, audio_lengths = _extract_features(self.valid_video_pipeline, self.valid_audio_pipeline, samples, self.args)
            return Batch(audios, audio_lengths, targets, target_lengths)
        if self.args.md == "v":
            videos, video_lengths = _extract_features(self.valid_video_pipeline, self.valid_audio_pipeline, samples, self.args)
            return Batch(videos, video_lengths, targets, target_lengths)
        if self.args.md == "av":
            audios, videos, audio_lengths, video_lengths = _extract_features(self.valid_video_pipeline, self.valid_audio_pipeline, samples, self.args)
            return AVBatch(audios, videos, audio_lengths, video_lengths, targets, target_lengths)
