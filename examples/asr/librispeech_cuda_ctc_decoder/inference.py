...
def collate_wrapper(batch):
    speeches, labels = [], []
    for i, (speech, _, label, _, _, _) in enumerate(batch):
...
def _parse_args():
    ...
    parser.add_argument("--nbest", type=int, default=10, help="number of best hypotheses to return")
    parser.add_argument(
        "--beam-size", type=int, default=10, help="beam size for determining number of hypotheses to store"
    )
    parser.add_argument(
        "--batch-size", type=int, default=4, help="batch size for decoding"
    )
    parser.add_argument(
