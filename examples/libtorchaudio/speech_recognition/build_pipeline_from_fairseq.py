...
def _parse_args():
    ...
    parser.add_argument(
        "--model-file", required=True, help="Path to the input pretrained weight file."
    )
    parser.add_argument(
        "--dict-dir",
        help=(
    ...
    parser.add_argument(
        "--quantize", action="store_true", help="Apply quantization to model."
    )
    parser.add_argument(
        "--optimize-for-mobile",
        action="store_true",
    ...
    def forward(self, audio_path: str) -> torch.Tensor:
        ...
        if sample_rate != 16000:
            waveform = torchaudio.functional.resample(
                waveform, float(sample_rate), 16000.0
            )
        return waveform
...
def _init_logging(debug=False):
    ...
    format_ = (
        "%(message)s"
        if not debug
        else "%(asctime)s: %(levelname)7s: %(funcName)10s: %(message)s"
    )
