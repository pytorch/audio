...
def get_dataset(
    dataset_type, root_dir, num_speakers, sample_rate, task=None, librimix_tr_split=None
):
...
def collate_fn_wsj0mix_train(samples: List[wsj0mix.SampleType], sample_rate, duration):
    ...
    for sample in samples:
        mix, src, mask = _fix_num_frames(
            sample, target_num_frames, sample_rate, random_start=True
        )
...
def collate_fn_wsj0mix_test(samples: List[wsj0mix.SampleType], sample_rate):
    ...
    for sample in samples:
        mix, src, mask = _fix_num_frames(
            sample, max_num_frames, sample_rate, random_start=False
        )
...
def get_collate_fn(dataset_type, mode, sample_rate=None, duration=4):
    ...
    if dataset_type in ["wsj0mix", "librimix"]:
        if mode == "train":
            ...
            return partial(
                collate_fn_wsj0mix_train, sample_rate=sample_rate, duration=duration
            )
