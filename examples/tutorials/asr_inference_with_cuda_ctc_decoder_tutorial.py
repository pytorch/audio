...
CUDA-based CTC beam search decoder. We demonstrate this on a pretrained `Zipformer <https://github.com/k2-fsa/icefall/tree/master/egs/librispeech/ASR/pruned_transducer_stateless7_ctc>`__ model from next-gen-kaldi project.
...
######################################################################
#
# We use the pretrained `Zipformer <https://huggingface.co/Zengwei/icefall-asr-librispeech-pruned-transducer-stateless7-ctc-2022-12-01>`__
...
cuda_decoder = cuda_ctc_decoder(tokens, nbest=10, beam_size=10, blank_skip_threshold=float(torch.log(torch.tensor(0.95))))
...
for beam_size in beam_sizes:
    beam_search_decoder = cuda_ctc_decoder(
        ...
        nbest=1,
        beam_size=beam_size,
        blank_skip_threshold=float(torch.log(torch.tensor(0.95)))
    )
...
# We recommend to set blank_skip_threshold=10 for cuda beam search decoder.
#
...
for blank_skip_prob in blank_skip_probs:
    beam_search_decoder = cuda_ctc_decoder(
        ...
        nbest=10,
        beam_size=10,
        blank_skip_threshold=float(torch.log(torch.tensor(blank_skip_prob)))
    )
