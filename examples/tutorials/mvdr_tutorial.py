...
**Author** ``Zhaoheng Ni <zni@fb.com>``\ \_\_
...
# ``ConferencingSpeech <https://github.com/ConferencingSpeech/ConferencingSpeech2021>``\ \_\_
# dataset.
# 
...
#    ``AISHELL-3 <https://www.openslr.org/93/>``\ \_\_ (Apache License
#    v.2.0)
# -  ``noise-sound-bible-0038.wav`` from
#    ``MUSAN <http://www.openslr.org/17/>``\ \_\_ (Attribution 4.0
#    International — CC BY 4.0) # noqa: E501
# 
...
######################################################################
# Section 3: Generate the Ideal Ratio Mask (IRM)
# ----------------------------------------------
# 


######################################################################
# Section 3.1: Load audio data
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# 
...
######################################################################
# To imrove the robustness of computation, it is recommended to use the
# double precision for the waveforms.
# 
...
######################################################################
# Section 3.2: Compute complex-valued Spectrums
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# 
...
######################################################################
# Section 3.2.1: Visualize mixture speech
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
# 

plot_specgram(spectrum_mix[0], "Spectrogram of Mixture Speech (dB)")
ipd.Audio(waveform_mix[0], rate=SAMPLE_RATE)


######################################################################
# Section 3.2.2: Visualize clean speech
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
# 

plot_specgram(spectrum_clean[0], "Spectrogram of Clean Speech (dB)")
ipd.Audio(waveform_clean[0], rate=SAMPLE_RATE)


######################################################################
# Section 3.2.3: Visualize noise
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
# 

plot_specgram(spectrum_noise[0], "Spectrogram of Noise (dB)")
ipd.Audio(waveform_noise[0], rate=SAMPLE_RATE)


######################################################################
# Section 3.3 Define the reference microphone
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# 
...
######################################################################
# Section 3.4 Compute IRMs for target speech and noise
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# 
...
######################################################################
# Section 3.4.1: Visualize IRM of target speech
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
# 
...
######################################################################
# Section 3.4.2: Visualize IRM of noise
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
# 
...
######################################################################
# Section 4: Beamforming using SoudenMVDR
# ---------------------------------------
# 


######################################################################
# Section 4.1: Compute power spectral density (PSD) matrices
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# 
...
######################################################################
# Section 4.3: Result for SoudenMVDR
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# 

plot_specgram(spectrum_souden, "Enhanced Spectrogram by SoudenMVDR (dB)")
ipd.Audio(waveform_souden, rate=SAMPLE_RATE)
waveform_souden = waveform_souden.reshape(1,-1)
print(f"Si-SNR result: {si_snr(waveform_souden, waveform_clean[0:1])}")


######################################################################
# Section 5: Beamforming using RTFMVDR
# ------------------------------------
# 


######################################################################
# Section 5.1: Compute power spectral density (PSD) matrices
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# 
...
######################################################################
# Section 5.4: Result for RTFMVDR with F.rtf_evd
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# 

plot_specgram(spectrum_rtf_evd, "Enhanced Spectrogram by RTFMVDR and F.rtf_evd (dB)")
ipd.Audio(waveform_rtf_evd, rate=SAMPLE_RATE)
waveform_rtf_evd = waveform_rtf_evd.reshape(1,-1)
print(f"Si-SNR result: {si_snr(waveform_rtf_evd, waveform_clean[0:1])}")


######################################################################
# Section 5.5: Result for RTFMVDR with F.rtf_power
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# 

plot_specgram(spectrum_rtf_power, "Enhanced Spectrogram by RTFMVDR and F.rtf_evd (dB)")
ipd.Audio(waveform_rtf_power, rate=SAMPLE_RATE)
waveform_rtf_power = waveform_rtf_power.reshape(1,-1)
print(f"Si-SNR result: {si_snr(waveform_rtf_power, waveform_clean[0:1])}")