...
class MockedSaveTest(PytorchTestCase):
    ...
    def test_wav(
        self, dtype, sample_rate, num_channels, channels_first, enc_params, mocked_write
    ):
    ...
    def test_sph(
        self, fmt, dtype, sample_rate, num_channels, channels_first, enc_params
    ):
    ...
    def test_flac(
        self, dtype, sample_rate, num_channels, channels_first, bits_per_sample
    ):
        """soundfile_backend.save passes default format and subtype (None-s) to
        soundfile.write when not WAV"""
        self.assert_non_wav(
