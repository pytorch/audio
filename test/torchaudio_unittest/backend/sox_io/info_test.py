...
class FileObjTestBase(TempDirMixin):
    def _gen_file(
        self, ext, dtype, sample_rate, num_channels, num_frames, *, comments=None
    ):
...
class TestFileObject(FileObjTestBase, PytorchTestCase):
    def _query_fileobj(
        ...
        path = self._gen_file(
            ext, dtype, sample_rate, num_channels, num_frames, comments=comments
        )
