...
class TestLoad(LoadTestBase):
    ...
    def test_amr_nb(self):
        ...
        self.assert_format(
            "amr-nb", sample_rate=8000, num_channels=1, bit_depth=32, duration=1
        )
...
class TestFileObject(TempDirMixin, PytorchTestCase):
    ...
    def test_fileobj(self, ext, compression):
        ...
        sox_utils.gen_audio_file(
            path, sample_rate, num_channels=2, compression=compression
        )
...
class TestLoadNoSuchFile(PytorchTestCase):
    def test_load_fail(self):
        ...
        with self.assertRaisesRegex(
            RuntimeError,
            "^Error loading audio file: failed to open file {0}$".format(path),
        ):
