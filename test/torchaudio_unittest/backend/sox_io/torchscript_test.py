...
class SoxIO(TempDirMixin, TorchaudioTestCase):
    ...
    def test_info_wav(self, dtype, sample_rate, num_channels):
        ...
        data = get_wav_data(
            dtype, num_channels, normalize=False, num_frames=1 * sample_rate
        )
    ...
    def test_load_wav(
        self, dtype, sample_rate, num_channels, normalize, channels_first
    ):
    ...
    def test_save_flac(self, sample_rate, num_channels, compression_level):
        ...
        py_save_func(
            py_path, expected, sample_rate, True, compression_level, None, None
        )
        ts_save_func(
            ts_path, expected, sample_rate, True, compression_level, None, None
        )
