...
class TestFunctional(common_utils.TorchaudioTestCase):
    ...
    @parameterized.expand(
        list(itertools.product([100, 440], [8000, 16000, 44100], [1, 2],)),
    ...
    def test_contrast(self):
        ...
        self.assert_batch_consistencies(F.contrast, waveform, enhancement_amount=80.0)
    ...
    def test_phaser(self):
        sample_rate = 44100
        waveform = common_utils.get_whitenoise(sample_rate=sample_rate, duration=5,)
...
class TestTransforms(common_utils.TorchaudioTestCase):
    ...
    def test_batch_InverseMelScale(self):
        ...
        # Single then transform then batch
        expected = torchaudio.transforms.InverseMelScale(n_stft, n_mels)(
            mel_spec
        ).repeat(3, 1, 1, 1)

        # Batch then transform
        computed = torchaudio.transforms.InverseMelScale(n_stft, n_mels)(
            mel_spec.repeat(3, 1, 1, 1)
        )
    ...
    def test_batch_TimeStretch(self):
        ...
        expected = torchaudio.transforms.TimeStretch(
            fixed_rate=rate, n_freq=1025, hop_length=512,
    ...
    def test_batch_Fade(self):
        ...
        # Single then transform then batch
        expected = torchaudio.transforms.Fade(fade_in_len, fade_out_len)(
            waveform
        ).repeat(3, 1, 1)

        # Batch then transform
        computed = torchaudio.transforms.Fade(fade_in_len, fade_out_len)(
            waveform.repeat(3, 1, 1)
        )
        self.assertEqual(computed, expected)
