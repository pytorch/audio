...
def run(exe_path, scp_path, out_dir, wave_len, num_outputs, remove_files, log_level):
    ...
    for _ in range(num_outputs):
        try:
            ...
            vtln_warp_factor = (
                random.uniform(0.0, 10.0) if random.random() < 0.3 else 1.0
            )
        ...
        inputs = {
            ...
            "frame_length": "%.4f"
            % (float(random.randint(3, wave_len - 1)) / 16000 * 1000),
            "frame_shift": "%.4f"
            % (float(random.randint(1, wave_len - 1)) / 16000 * 1000),
        ...
        try:
            if log_level == "INFO":
                ...
                subprocess.call(
                    arg, stderr=open(os.devnull, "wb"), stdout=open(os.devnull, "wb")
                )
...
def decode(fn, sound_path, exe_path, scp_path, out_dir):
    ...
    fn_split = fn.split("-")
    assert len(fn_split) == len(arr), "Len mismatch: {} and {}".format(
        len(fn_split), len(arr)
    )
    inputs = {arr[i]: utils.parse(fn_split[i]) for i in range(len(arr))}

    # print flags for C++
    s = " ".join(
        ["--" + arr[i].replace("_", "-") + "=" + fn_split[i] for i in range(len(arr))]
    )
    logging.info(
        exe_path + " --dither=0.0 --debug-mel=true " + s + " " + scp_path + " " + out_fn
    )
    logging.info()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to the compute-fbank-feats executable.",
    )
    parser.add_argument(
        "--scp_path",
        type=str,
        required=True,
        help='Path to the scp file. An example of its contents would be \
    ...
    parser.add_argument(
        ...
        type=bool,
        default=False,
        help="Whether to run the decode or run function.",
    )
