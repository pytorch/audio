    ...
    def setUpClass(cls):
        ...
        for i, utterance_id in enumerate(cls.utterance_ids):
            ...
            file_dir = os.path.join(
                base_dir, str(utterance_id[0]), str(utterance_id[1])
            )
            os.makedirs(file_dir, exist_ok=True)
            path = os.path.join(file_dir, filename)

            data = get_whitenoise(
                sample_rate=24000, duration=2, n_channels=1, dtype="int16", seed=i
            )
            save_wav(path, data, 24000)
            cls.data.append(normalize_wav(data))

            original_text_filename = (
                f'{"_".join(str(u) for u in utterance_id)}.original.txt'
            )
            ...
            normalized_text_filename = (
                f'{"_".join(str(u) for u in utterance_id)}.normalized.txt'
            )
