...
def batch_SDR_torch(estimation, origin, mask=None):
    ...
    assert (
        batch_size_est == batch_size_ori
    ), "Estimation and original sources should have same shape."
    assert (
        nsource_est == nsource_ori
    ), "Estimation and original sources should have same shape."
    assert (
        nsample_est == nsample_ori
    ), "Estimation and original sources should have same shape."

    assert (
        nsource_est < nsample_est
    ...
    # zero mean signals
    estimation = estimation - torch.mean(estimation, 2, keepdim=True).expand_as(
        estimation
    )
