...
class TestFunctional(common_utils.TorchaudioTestCase):
    ...
    def assert_batch_consistency(
        self, functional, batch, *args, atol=1e-8, rtol=1e-5, seed=42, **kwargs
    ):
        n = batch.size(0)
    ...
    def test_amplitude_to_DB(self):
        ...
        self.assert_batch_consistency(
            F.amplitude_to_DB, spec, amplitude_mult, amin, db_mult, top_db=None
        )
        self.assert_batch_consistency(
            F.amplitude_to_DB, spec, amplitude_mult, amin, db_mult, top_db=40.0
        )
    ...
    def test_amplitude_to_DB_itemwise_clamps(self):
        ...
        batchwise_dbs = F.amplitude_to_DB(
            spec, amplitude_mult, amin, db_mult, top_db=top_db
        )
        itemwise_dbs = torch.stack(
            [
                F.amplitude_to_DB(item, amplitude_mult, amin, db_mult, top_db=top_db)
                for item in spec
            ]
        )
    ...
    def test_amplitude_to_DB_not_channelwise_clamps(self):
        ...
        specwise_dbs = F.amplitude_to_DB(
            spec, amplitude_mult, amin, db_mult, top_db=top_db
        )
        channelwise_dbs = torch.stack(
            [
                F.amplitude_to_DB(
                    spec[:, i], amplitude_mult, amin, db_mult, top_db=top_db
                )
    ...
    def test_sliding_window_cmn(self, center, norm_vars):
        ...
        self.assert_batch_consistency(
            F.sliding_window_cmn, spectrogram, center=center, norm_vars=norm_vars
        )
    ...
    def test_filtfilt(self):
        ...
        itemwise_output = torch.stack(
            [F.filtfilt(x[i], a[i], b[i]) for i in range(self.batch_size)]
        )
