...
class TestDetectPitchFrequency(common_utils.TorchaudioTestCase):
    ...
    def test_pitch(self, frequency):
        ...
        freq = torchaudio.functional.detect_pitch_frequency(
            test_sine_waveform, sample_rate
        )
...
class TestDB_to_amplitude(common_utils.TorchaudioTestCase):
    def test_DB_to_amplitude(self):
        ...
        db = F.amplitude_to_DB(
            torch.abs(x), multiplier, amin, db_multiplier, top_db=None
        )
...
@pytest.mark.parametrize(
    "complex_tensor", [torch.randn(1, 2, 1025, 400, 2), torch.randn(1025, 400, 2)]
)
...
def test_complex_norm(complex_tensor, power):
    ...
    torch.testing.assert_allclose(
        norm_tensor, expected_norm_tensor, atol=1e-5, rtol=1e-5
    )


@pytest.mark.parametrize(
    "specgram", [torch.randn(2, 1025, 400), torch.randn(1, 201, 100)]
)
