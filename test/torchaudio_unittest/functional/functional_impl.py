...
class LfilterCPP(common_utils.TestBaseMixin):
    def test_simple(self):
        """
        Create a very basic signal,
        Then make a simple 4th order delay
        ...
        b_coeffs = torch.tensor([0, 0, 0, 1], dtype=self.dtype, device=self.device)
        a_coeffs = torch.tensor([1, 0, 0, 0], dtype=self.dtype, device=self.device)
        output_waveform = torch.ops.torchaudio.functional_lfilter(waveform, a_coeffs, b_coeffs, False)
    ...
    def test_clamp(self):
        ...
        b_coeffs = torch.tensor([1, 0], dtype=self.dtype, device=self.device)
        a_coeffs = torch.tensor([1, -0.95], dtype=self.dtype, device=self.device)
        output_signal = torch.ops.torchaudio.functional_lfilter(input_signal, a_coeffs, b_coeffs, True)
        assert output_signal.max() <= 1
        output_signal = torch.ops.torchaudio.functional_lfilter(input_signal, a_coeffs, b_coeffs, False)
        assert output_signal.max() > 1


    def test_match(self):
        input_signal = torch.ones(1, 256, dtype=self.dtype, device=self.device)
        b_coeffs = torch.tensor([1, 0], dtype=self.dtype, device=self.device)
        a_coeffs = torch.tensor([1, -0.95], dtype=self.dtype, device=self.device)
        cpp_result = torch.ops.torchaudio.functional_lfilter(input_signal, a_coeffs, b_coeffs, True)
        py_result = F.lfilter(input_signal, a_coeffs, b_coeffs, True)

        self.assertEqual(py_result, cpp_result)
