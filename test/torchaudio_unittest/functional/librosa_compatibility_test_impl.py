...
class Functional(TestBaseMixin):
    ...
    def test_create_mel_fb(
        ...
        if norm == "slaney" and StrictVersion(librosa.__version__) < StrictVersion(
            "0.7.2"
        ):
    ...
    def test_amplitude_to_DB_power(self):
        ...
        spec = get_spectrogram(
            get_whitenoise(device=self.device, dtype=self.dtype), power=2
        )
    ...
    def test_amplitude_to_DB(self):
        ...
        spec = get_spectrogram(
            get_whitenoise(device=self.device, dtype=self.dtype), power=1
        )
        result = F.amplitude_to_DB(spec, multiplier, amin, db_multiplier, top_db)
...
class FunctionalComplex(TestBaseMixin):
    ...
    def test_phase_vocoder(self, rate):
        ...
        # match with librosa.
        spec = torch.randn(
            num_freq, num_frames, device=self.device, dtype=torch.complex128
        )
        phase_advance = torch.linspace(
            0, np.pi * hop_length, num_freq, device=self.device, dtype=torch.float64
        )[..., None]

        stretched = F.phase_vocoder(spec, rate=rate, phase_advance=phase_advance)

        expected_stretched = librosa.phase_vocoder(
            spec.cpu().numpy(), rate=rate, hop_length=hop_length
        )
