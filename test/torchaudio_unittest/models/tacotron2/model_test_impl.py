...
def _get_decoder_model(
    n_mels=80, encoder_embedding_dim=512, decoder_max_step=2000, gate_threshold=0.5
):
    model = _Decoder(
        n_mels=n_mels,
        n_frames_per_step=1,
...
class Tacotron2DecoderTests(TorchscriptConsistencyMixin):
    def test_decoder_torchscript_consistency(self):
        ...
        model = _get_decoder_model(
            n_mels=n_mels, encoder_embedding_dim=encoder_embedding_dim
        )
