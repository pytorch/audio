...
def _load_config(*paths):
    with open(
        f'{get_asset_path("wav2vec2", "huggingface", *paths)}.json', "r"
    ) as file_:
...
class TestHFIntegration(TorchaudioTestCase):
    ...
    def _test_import_pretrain(self, original, imported, config):
        ...
        for original_, imported_ in zip(
            original.encoder.layers, imported.encoder.transformer.layers
        ):
    ...
    def _test_import_finetune(self, original, imported, config):
        ...
        mask = (
            torch.arange(num_frames).expand(batch_size, num_frames) < lengths[:, None]
        )
    ...
    def _test_recreate(self, imported, reloaded, config):
        ...
        self.assertEqual(ref, hyp)
        # Encoder Transformer Layer
        for imported_, reloaded_ in zip(
            imported.encoder.transformer.layers, reloaded.encoder.transformer.layers
        ):
            b, l, e = 16, 3, config["hidden_size"]
            x = torch.randn(b, l, e)
            mask = torch.randn(b, 1, l, l)
