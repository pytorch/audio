...
@skipIf(
    ...
    and sys.version_info.minor in [6, 7],
...
class TestSoxEffectsDataset(TempDirMixin, PytorchTestCase):
    ...
    def _generate_dataset(self, num_samples=128):
        ...
        for i in range(num_samples):
            ...
            data = get_whitenoise(
                n_channels=2, sample_rate=sample_rate, duration=1, dtype=dtype
            )
...
class TestProcessPoolExecutor(TempDirMixin, PytorchTestCase):
    ...
    def setUp(self):
        ...
        for i in range(10):
            ...
            data = get_whitenoise(
                n_channels=1, sample_rate=sample_rate, duration=1, dtype="float"
            )
            save_wav(path, data, sample_rate)
