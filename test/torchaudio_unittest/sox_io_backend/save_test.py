...
class SaveTestBase(TempDirMixin, PytorchTestCase):
    ...
    def assert_mp3(self, sample_rate, num_channels, bit_rate, duration):
        ...
        data = get_wav_data(
            "float32", num_channels, normalize=True, num_frames=duration * sample_rate
        )
    ...
    def assert_flac(self, sample_rate, num_channels, compression_level, duration):
        ...
        sox_utils.convert_audio_file(
            src_path, flc_path_sox, compression=compression_level
        )
    ...
    def _assert_vorbis(self, sample_rate, num_channels, quality_level, duration):
        ...
        # 1. Generate original wav
        data = get_wav_data(
            "int16", num_channels, normalize=False, num_frames=duration * sample_rate
        )
        save_wav(src_path, data, sample_rate)
        ...
        atol = 1.0e-4
        max_failure_allowed = (
            0.01  # this percent of samples are allowed to outside of atol.
        )
...
class TestSave(SaveTestBase):
    ...
    @parameterized.expand(
        list(itertools.product(["float32"], [16000], [2],)), name_func=name_func
    )
    ...
    def test_flac_large(self, sample_rate, num_channels, compression_level):
        ...
        two_hours = 2 * 60 * 60
        self.assert_flac(
            sample_rate, num_channels, compression_level, duration=two_hours
        )
