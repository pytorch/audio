...
class TransformsTestBase(TestBaseMixin):
    ...
    def test_Spectrogram(self, n_fft, hop_length, power):
        ...
        result = T.Spectrogram(n_fft=n_fft, hop_length=hop_length, power=power,).to(
    ...
    def test_magnitude_to_db(self):
        spectrogram = get_spectrogram(get_whitenoise(), n_fft=400, power=2).to(
            self.device, self.dtype
        )
        result = T.AmplitudeToDB("magnitude", 80.0).to(self.device, self.dtype)(
            spectrogram
        )[0]
    ...
    def test_power_to_db(self):
        ...
        result = T.AmplitudeToDB("power", 80.0).to(self.device, self.dtype)(
            spectrogram
        )[0]
    ...
    def test_mfcc(self, n_fft, hop_length, n_mels, n_mfcc):
        sample_rate = 16000
        waveform = get_whitenoise(sample_rate=sample_rate, n_channels=1).to(
            self.device, self.dtype
        )
        result = T.MFCC(
            sample_rate=sample_rate,
