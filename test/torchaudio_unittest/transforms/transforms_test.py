...
class Tester(common_utils.TorchaudioTestCase):
    ...
    volume = 0.3
    waveform = torch.cos(
        2 * math.pi * torch.arange(0, 4 * sample_rate).float() * freq / sample_rate
    )
    ...
    def test_mu_law_companding(self):
        ...
        self.assertTrue(
            waveform_mu.min() >= 0.0 and waveform_mu.max() <= quantization_channels
        )
    ...
    def test_mel2(self):
        ...
        fb_matrix_transform = transforms.MelScale(
            n_mels=100, sample_rate=16000, f_min=0.0, f_max=None, n_stft=400
        )
    ...
    def test_resample_size(self):
        ...
        with self.assertRaises(ValueError):
            torchaudio.transforms.Resample(
                sample_rate, upsample_rate, resampling_method=invalid_resampling_method
            )
    ...
    def test_compute_deltas(self):
        ...
        self.assertTrue(
            computed.shape == specgram.shape, (computed.shape, specgram.shape)
        )
...
class SmokeTest(common_utils.TorchaudioTestCase):
    def test_spectrogram(self):
        specgram = transforms.Spectrogram(
            center=False, pad_mode="reflect", onesided=False
        )
    ...
    def test_melspectrogram(self):
        melspecgram = transforms.MelSpectrogram(
            center=True, pad_mode="reflect", onesided=False
        )
