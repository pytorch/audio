...
namespace cu_ctc
{
    ...
    find_divisor(unsigned int &mul, unsigned int &shr, unsigned int denom)
    {
        if (denom == 1)
        {
            mul = 0;
    ...
    __host__ __device__ __forceinline__
        ...
        void
        fast_divmod(int &quo, int &rem, int src, int div, unsigned int mul,
                    unsigned int shr)
    {
    ...
    // For long int input
    __host__ __device__ __forceinline__ void fast_divmod(int &quo, int64_t &rem,
                                                         int64_t src, int div,
                                                         unsigned int mul,
                                                         unsigned int shr)
    {
    ...
    struct FastDivmod
    {
        ...
        __host__ __device__ __forceinline__ void
        operator()(int &quotient, int &remainder, int dividend) const
        {
        ...
        __host__ __device__ __forceinline__ int divmod(int &remainder,
                                                       int dividend) const
        {
            int quotient;
            fast_divmod(quotient, remainder, dividend, divisor, multiplier,
        ...
        /// computationally inexpensive.
        __host__ __device__ __forceinline__ void
        operator()(int &quotient, int64_t &remainder, int64_t dividend) const
        {
            fast_divmod(quotient, remainder, dividend, divisor, multiplier,
                        shift_right);
        }
        ...
        /// Computes integer division and modulus using precomputed values. This is
        /// computationally inexpensive.
        __host__ __device__ __forceinline__ int divmod(int64_t &remainder,
