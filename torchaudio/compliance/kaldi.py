...
def spectrogram(
    ...
    power_spectrum = torch.max(
        fft.pow(2).sum(2), epsilon
    ).log()  # size (m, padded_window_size // 2 + 1)
...
def mfcc(
    ...
    ), f"num_ceps cannot be larger than num_mel_bins: {num_ceps} vs {num_mel_bins}"
