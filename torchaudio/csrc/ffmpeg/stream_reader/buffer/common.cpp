...
std::tuple<torch::Tensor, bool> get_image_buffer(
  ...
  if (desc->flags & AV_PIX_FMT_FLAG_PLANAR) {
    auto buffer = get_buffer({num_frames, channels, height, width}, device, dtype);
    return std::make_tuple(buffer, true);
  }
  auto buffer = get_buffer({num_frames, height, width, channels}, device, dtype);
...
void write_p010_cuda(AVFrame* pFrame, torch::Tensor& yuv) {
  ...
  torch::Tensor uv = get_buffer({1, height / 2, width / 2, 2}, yuv.device(), torch::kInt16);
  {
  ...
  // shorts
  yuv.index_put_({Slice(), Slice(1, 3), Slice(None, None, 2), Slice(None, None, 2)}, uv);
  yuv.index_put_({Slice(), Slice(1, 3), Slice(1, None, 2), Slice(None, None, 2)}, uv);
  yuv.index_put_({Slice(), Slice(1, 3), Slice(None, None, 2), Slice(1, None, 2)}, uv);
  yuv.index_put_({Slice(), Slice(1, 3), Slice(1, None, 2), Slice(1, None, 2)}, uv);
  // correct for int16
  yuv += 32768;
}
#endif
