...
class RNNTLossFunction : public torch::autograd::Function<RNNTLossFunction> {
  public:
    static torch::autograd::tensor_list forward(
      ...
      torch::Tensor undef;
      auto result = rnnt_loss(logits, targets, src_lengths, tgt_lengths, blank, clamp,
  ...
  static torch::autograd::tensor_list backward(
    ...
    auto grad = saved[0];auto grad_out = grad_outputs[0].view({-1, 1, 1, 1});
...
std::tuple<torch::Tensor, c10::optional<torch::Tensor>> rnnt_loss_autograd(
  // torch::Tensor rnnt_loss_autograd(
    torch::Tensor& logits,
    const torch::Tensor& targets,
    const torch::Tensor& src_lengths,
    const torch::Tensor& tgt_lengths,
    ...
    bool fused_log_smax = true,
    bool reuse_logits_for_grads = true) {
  auto results = RNNTLossFunction::apply(logits, targets, src_lengths, tgt_lengths, blank, clamp,
                                       fused_log_smax, reuse_logits_for_grads);
