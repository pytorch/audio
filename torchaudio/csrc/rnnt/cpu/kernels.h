...
HOST_AND_DEVICE void ComputeGradientsElement(
  ...
  for (int d = 0; d < D; ++d) {
    ...
    } else { // Monotonic RNNT case
      CAST_DTYPE g = cost + CAST_DTYPE(logits[b_t_u_d]);
      if (d == blank && t == T - 1 && u == U - 1) {  // last blank transition.
        g = g + alphas[idx_b_t_u];
      } else if (t < T - 1 && d == blank) {
        g = g + alphas[idx_b_t_u];
        if (idx_b_tp1_u != -1) {
          g = g + betas[idx_b_tp1_u];
        }
      } else if (u < U - 1 && t < T - 1 && d == targets[idxr2(bTgt, u)]) {
        g = g + alphas[idx_b_t_u];
        if (idx_b_tp1_up1 != -1) {
          g = g + betas[idx_b_tp1_up1];
        }
      }
      else {
        g = g + CAST_DTYPE(-INFINITY);
      }
      gradients[b_t_u_d] = -std::exp(g);
    } // end of monotonic else
