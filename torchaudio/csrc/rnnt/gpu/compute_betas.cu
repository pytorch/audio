...
torch::Tensor compute_betas_sparse(
    const torch::Tensor& logits,
    const torch::Tensor& targets,
    const torch::Tensor& src_lengths,
    const torch::Tensor& tgt_lengths,
  ...
  // Only support float, this is mainly to enable easy
  // unit-testing
  ComputeBetasSparse</*DTYPE=*/float, /*CAST_DTYPE=*/float>(
      /*workspace=*/workspace,
      /*logits=*/logits.data<float>(),
      /*targets=*/targets.data<int>(),
      /*src_lengths=*/src_lengths.data<int>(),
...
TORCH_LIBRARY_FRAGMENT(torchaudio, m) {
    m.def("rnnt_loss_betas_sparse(Tensor logits,"
                                 "Tensor targets,"
                                 "Tensor src_lengths,"
                                 "Tensor tgt_lengths,"
                                 "int blank,"
