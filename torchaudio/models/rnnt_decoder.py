...
def get_hypo_predictor_out(hypo: Hypothesis) -> torch.Tensor:
    return hypo[1]


def get_hypo_state(hypo: Hypothesis) -> List[List[torch.Tensor]]:
    return hypo[2]
...
def get_hypo_key(hypo: Hypothesis) -> str:
    return str(hypo[0])


def _batch_state(hypos: List[Hypothesis]) -> List[List[torch.Tensor]]:
    states: List[List[torch.Tensor]] = []
    for i in range(len(get_hypo_state(hypos[0]))):
        batched_state_components: List[torch.Tensor] = []
        for j in range(len(get_hypo_state(hypos[0])[i])):
            batched_state_components.append(torch.cat([get_hypo_state(hypo)[i][j] for hypo in hypos]))
...
def _default_hypo_sort_key(hypo: Hypothesis) -> float:
    return get_hypo_score(hypo) / (len(get_hypo_tokens(hypo)) + 1)


def _compute_updated_scores(
    hypos: List[Hypothesis], next_token_probs: torch.Tensor, beam_width: int,
) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    hypo_scores = torch.tensor([get_hypo_score(h) for h in hypos]).unsqueeze(1)
...
def _remove_hypo(hypo: Hypothesis, hypo_list: List[Hypothesis]) -> None:
    for i, elem in enumerate(hypo_list):
        if get_hypo_key(hypo) == get_hypo_key(elem):
            del hypo_list[i]
            break
...
class RNNTBeamSearch(torch.nn.Module):
    ...
    def _init_b_hypos(self, hypo: Optional[Hypothesis], device: torch.device) -> List[Hypothesis]:
        if hypo is not None:
            token = get_hypo_tokens(hypo)[-1]
            state = get_hypo_state(hypo)
        else:
            token = self.blank
            state = None
    ...
    def _gen_next_token_probs(
        ...
        one_tensor = torch.tensor([1], device=device)
        predictor_out = torch.stack([get_hypo_predictor_out(h) for h in hypos], dim=0)
        joined_out, _, _ = self.model.join(
            enc_out, one_tensor, predictor_out, torch.tensor([1] * len(hypos), device=device),
    ...
    def _gen_b_hypos(
        ...
        for i in range(len(a_hypos)):
            h_a = a_hypos[i]
            append_blank_score = get_hypo_score(h_a) + next_token_probs[i, -1]
            if get_hypo_key(h_a) in key_to_b_hypo:
                h_b = key_to_b_hypo[get_hypo_key(h_a)]
                _remove_hypo(h_b, b_hypos)
                score = float(torch.tensor(get_hypo_score(h_b)).logaddexp(append_blank_score))
            else:
        ...
        _, sorted_idx = torch.tensor([get_hypo_score(hypo) for hypo in b_hypos]).sort()
    ...
    def _gen_a_hypos(
        ...
        (nonblank_nbest_scores, nonblank_nbest_hypo_idx, nonblank_nbest_token,) = _compute_updated_scores(
            a_hypos, next_token_probs, beam_width
        )

        if len(b_hypos) < beam_width:
            b_nbest_score = -float("inf")
        else:
            b_nbest_score = get_hypo_score(b_hypos[-beam_width])
    ...
    def _gen_new_hypos(
        self, base_hypos: List[Hypothesis], tokens: List[int], scores: List[float], t: int, device: torch.device,
    ) -> List[Hypothesis]:
    ...
    def _search(self, enc_out: torch.Tensor, hypo: Optional[Hypothesis], beam_width: int,) -> List[Hypothesis]:
        ...
        for t in range(n_time_steps):
            ...
            while a_hypos:
                ...
                a_hypos = self._gen_a_hypos(a_hypos, b_hypos, next_token_probs, t, beam_width, device,)
                if a_hypos:
                    symbols_current_t += 1
