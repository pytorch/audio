...
class Stretch2d(nn.Module):
    ...
    def forward(self, specgram: Tensor) -> Tensor:
        ...
        return specgram.repeat_interleave(self.freq_scale, -2).repeat_interleave(
            self.time_scale, -1
        )
...
class UpsampleNetwork(nn.Module):
    ...
    def forward(self, specgram: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        upsampling_output = upsampling_output.squeeze(1)[
...
class WaveRNN(nn.Module):
    ...
    def forward(self, waveform: Tensor, specgram: Tensor) -> Tensor:
        ...
        h1 = torch.zeros(
            1, batch_size, self.n_rnn, dtype=waveform.dtype, device=waveform.device
        )
        h2 = torch.zeros(
            1, batch_size, self.n_rnn, dtype=waveform.dtype, device=waveform.device
        )
    ...
    @torch.jit.export
    def infer(
        self, specgram: Tensor, lengths: Optional[Tensor] = None
