...
class _EnglishCharProcessor(Tacotron2TTSBundle.TextProcessor):
    ...
    def __call__(self, texts: Union[str, List[str]]) -> Tuple[Tensor, Tensor]:
        if isinstance(texts, str):
            texts = [texts]
        indices = [
            [self._mapping[c] for c in t.lower() if c in self._mapping] for t in texts
        ]
...
@dataclass
class _Tacotron2WaveRNNCharBundle(
...
@dataclass
class _Tacotron2WaveRNNPhoneBundle(
    _WaveRNNMixin, _Tacotron2Mixin, _PhoneMixin, Tacotron2TTSBundle
):
    pass


@dataclass
class _Tacotron2GriffinLimCharBundle(
    _GriffinLimMixin, _Tacotron2Mixin, _CharMixin, Tacotron2TTSBundle
):
    pass


@dataclass
class _Tacotron2GriffinLimPhoneBundle(
    _GriffinLimMixin, _Tacotron2Mixin, _PhoneMixin, Tacotron2TTSBundle
):
    pass
